...
spells <- tribble(
  ~mana, ~damage, ~heal, ~duration_shield, ~duration_poison, ~duration_recharge,
   53, 4, 0, 0, 0, 0,
   73, 2, 2, 0, 0, 0,
  113, 0, 0, 6, 0, 0,
  173, 0, 0, 0, 6, 0,
...
for (i in 1:(player_hit_points + boss$`Hit Points`)) {
  if (i %% 2 == 1) {
    ...
    strats %>%
      ...
      mutate(total_mana = total_mana + mana) %>%
      ...
      summarise(total_mana = min(total_mana), .groups = "drop") -> strats
    if (nrow(strats) == 0)
      break
    # print(nrow(strats))
  } else
